		...
		if($request->getMethod()=='POST'){
			...
			if($_SESSION['userId']){
				try{
					...
					$persona = new Vehiculos();
					$persona->placa=$postData['placa'];
					$persona->interno = $postData['interno'];
					$persona->km = $postData['km'];
					$persona->tipvinculacionid = $postData['tipvinculacionid'];
					$persona->licenciatransito = $postData['licenciatransito'];
					$persona->capacidad = $postData['capacidad'];
					$persona->marcid = $postData['marcid'];
					$persona->linea = $postData['linea'];
					$persona->modelo = $postData['modelo'];
					$persona->cilindraje = $postData['cilindraje'];
					$persona->potencia = $postData['potencia'];
					$persona->colorid = $postData['colorid'];
					$persona->servid = $postData['servid'];
					$persona->clasid = $postData['clasid'];
					$persona->carroceria = $postData['carroceria'];
					$persona->combustibleid = $postData['combustibleid'];
					$persona->puertas = $postData['puertas'];
					$persona->nummotor = $postData['nummotor'];
					$persona->vin = $postData['vin'];
					$persona->numserie = $postData['numserie'];
					$persona->numchasis = $postData['numchasis'];
					$persona->fechamatricula = $postData['fechamatricula'];
					$persona->fechaexpedicion = $postData['fechaexpedicion'];
					$persona->orgtransitoid = $postData['orgtransitoid'];
